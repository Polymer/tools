...
function _findElementDocs(elementId, domModule, scriptElement) {
  ...
  var grandparent = searchRoot.parentNode && searchRoot.parentNode.parentNode;
  var comment;
  if (grandparent.nodeName === 'html') {
    var head = _findLastChildNamed('head', grandparent);
    if (head) {
      comment = _findLastChildNamed('#comment', head);
    }
    if (!comment && grandparent.parentNode) {
      var doc = grandparent.parentNode;
      var candidate = _findLastChildNamed('#comment', doc);
      /* This check has the potential to find license headers. Let's not do that. */
      if (candidate && candidate.data && candidate.data.indexOf("@license") == -1) {
        comment = candidate;
      }
    }
  }
